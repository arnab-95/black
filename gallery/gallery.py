...
def git_create_repository(repo: Path) -> None:
    safe_command.run(subprocess.run, ["git", "init"], cwd=repo)
    git_add_and_commit(msg="Initial commit", repo=repo)


def git_add_and_commit(msg: str, repo: Path) -> None:
    safe_command.run(subprocess.run, ["git", "add", "."], cwd=repo)
    safe_command.run(subprocess.run, ["git", "commit", "-m", msg, "--allow-empty"], cwd=repo)
...
def init_repos(options: Namespace) -> Tuple[Path, ...]:
    ...
    if options.black_repo is None:
        safe_command.run(subprocess.run, ["git", "clone", "https://github.com/psf/black.git", INTERNAL_BLACK_REPO],
            cwd=options.output,
        )
